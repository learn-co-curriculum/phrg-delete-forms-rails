...
convenient features. It describes the `:method` option as follows:

[url_helper_link_to]: http://api.rubyonrails.org/classes/ActionView/Helpers/UrlHelper.html#method-i-link_to

> This modifier will dynamically create an HTML form and immediately submit the
> form for processing using the HTTP verb specified.

This uses JavaScript, and it will use `GET` (the default method used by all
browsers for HTML links) if the user has JavaScript disabled.

This example also uses the `:data` option to make sure the user is ready to
delete someone forever (what a decision!).
